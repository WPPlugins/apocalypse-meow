...
class ajax {
	...
	public static function init() {
		...
		foreach (static::ACTIONS as $action) {
			add_action("wp_ajax_meow_ajax_$action", array($class, $action));
		}
	...
	protected static function parse(&$data, $nonce=true, $pro=false) {
		...
		if ($pro && !options::is_pro()) {
			$out['errors']['pro'] = __('A premium license is required.', 'apocalypse-meow');
			$out['status'] = 403;
		}
	...
	public static function activity_csv() {
		...
		$out['data'] = array(
			'download'=>'',
			'downloadName'=>common\sanitize::domain(site_url()) . '_meow-' . time() . '.csv'
		);
			...
			if (!is_array($dbResult) || !count($dbResult)) {
				...
				foreach ($dbResult as $Row) {
					...
					if ('fail' === $Row['type']) {
						...
						common\ref\mb::ucfirst($Row['type']);
					}
		...
		if (!count($out['errors'])) {
			...
			// Maybe a real one.
				...
				if ($license->is_license()) {
					...
					$out['errors']['license'] = __('The license is not valid.', 'apocalypse-meow');
				}
			}
		}
	 ...
	 * Save Settings
	 *
	 * This handles all settings except "license", which gets its own
	 * page and callback.
	 *
	 * @return void Nothing.
	 */
	public static function settings() {
		...
		if (!count($out['errors'])) {
			...
			if (isset($_POST['login']['exempt'])) {
				// This should be an array.
				common\ref\cast::to_string($_POST['login']['exempt'], true);
				$_POST['login']['exempt'] = explode("\n", $_POST['login']['exempt']);
			}
			...
			$data['license'] = options::get('license');
			...
			$out['data']['login']['exempt'] = trim(implode("\n", $out['data']['login']['exempt']));
	...
	public static function tools_admin() {
		...
		if (!count($out['errors'])) {
			...
			foreach ($users as $user_old) {
				...
				$user_new = isset($_POST[$user_old]) ? $_POST[$user_old] : '';
				...
				foreach ($out['data']['sessions'] as $k=>$v) {
					usort(
						...
						function($a, $b) {
							...
							return ($a['date_expires'] < $b['date_expires']) ? -1 : 1;
						}
					);
				}
			}
