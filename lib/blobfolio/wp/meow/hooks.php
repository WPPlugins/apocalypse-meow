...
class hooks {
	...
	public static function init() {
		...
		add_action('meow_log_ban', array($class, 'log_ban'), 5, 1);
		add_action('meow_log_fail', array($class, 'log_fail'), 5, 1);
		add_action('meow_log_success', array($class, 'log_success'), 5, 1);

		// Filters that Meow returns data on. Do these at the normal time.
		add_filter('meow_failures_remaining', array($class, 'failures_remaining'), 10, 2);
		add_filter('meow_is_banned', array($class, 'is_banned'), 10, 2);
		add_filter('meow_is_whitelisted', array($class, 'is_whitelisted'), 10, 2);
	}
